...
import { ShieldCheck, Layers, Award, TrendingUp, DollarSign, Users, ArrowUpRight, ChevronRight, FileText } from 'lucide-react';
...
export const DirectorDashboardView: React.FC<DirectorDashboardViewProps> = () => {
  ...
  return (
    <div className="space-y-6 w-full min-w-0">
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 md:p-10 shadow-xl relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="relative z-10 space-y-2">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 bg-white/10 rounded-full text-xs font-bold text-brand-300 backdrop-blur-sm font-montserrat">
            <Award className="w-4 h-4 text-brand-400" /> Dirección de Inversiones & Portafolio
          </div>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight font-montserrat">
            Hola, {user?.name?.split(' ')[0]} 👋
          </h1>
          <p className="text-xs sm:text-sm text-slate-300 max-w-xl">
            Control de activos bajo gestión (AUM), liquidación de rentabilidades y proyecciones de inversión.
          </p>
        </div>
        ...
        <div className="relative z-10 flex flex-wrap items-center gap-3 shrink-0">
          ...
          >
            <FileText className="w-4 h-4" /> Gestionar Solicitudes
          </Link>
        </div>
      </div>
      ...
      {cards && (
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 w-full min-w-0">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-1 min-w-0 overflow-hidden">
            ...
            <span className="text-lg sm:text-xl xl:text-2xl font-extrabold text-emerald-700 block tracking-tight truncate font-mono" title={formatCardCurrency(cards.total_aum)}>
              {formatCardCurrency(cards.total_aum)}
            </span>
            <span className="text-[11px] text-slate-500 font-semibold">
              {cards.total_contratos_activos} contratos vigentes
            </span>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-1 min-w-0 overflow-hidden">
            <span className="text-[11px] text-slate-400 font-bold uppercase tracking-wider block font-montserrat">
              Rendimiento Est. Mensual
            </span>
            <span className="text-lg sm:text-xl xl:text-2xl font-extrabold text-slate-900 block tracking-tight truncate font-mono" title={formatCardCurrency(cards.rendimiento_mensual_estimado)}>
              {formatCardCurrency(cards.rendimiento_mensual_estimado)}
            </span>
            <span className="text-[11px] text-slate-500 font-semibold">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-1 min-w-0 overflow-hidden">
            <span className="text-[11px] text-slate-400 font-bold uppercase tracking-wider block font-montserrat">
              Proyectado a 30 Días
            </span>
            <span className="text-lg sm:text-xl xl:text-2xl font-extrabold text-indigo-700 block tracking-tight truncate font-mono" title={formatCardCurrency(cards.proyectado_30d)}>
              {formatCardCurrency(cards.proyectado_30d)}
            </span>
            <span className="text-[11px] text-slate-500 font-semibold">
              Compromiso a próximo cierre
            </span>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-1 min-w-0 overflow-hidden">
            ...
            <span className="text-lg sm:text-xl xl:text-2xl font-extrabold text-amber-600 block tracking-tight truncate font-mono" title={formatCardCurrency(cards.solicitudes_pendientes_monto)}>
              {formatCardCurrency(cards.solicitudes_pendientes_monto)}
            </span>
            <span className="text-[11px] text-slate-500 font-semibold">
              {cards.solicitudes_pendientes_count} solicitudes pendientes
            </span>
          </div>
      ...
      {/* Gráficas Ejecutivas del Directivo */}
