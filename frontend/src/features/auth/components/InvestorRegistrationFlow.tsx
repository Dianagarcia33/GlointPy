...
export const InvestorRegistrationFlow = () => {
    ...
    const uploadKycDocsMutation = useMutation({
        ...
        onSuccess: (paths: string[]) => {
            setKycPaths(paths);
            setStep(3); // Advance to final form step
        },
        onError: (error: any) => {
            ...
            {step === 1 && (
                <div className="space-y-6 animate-fadeIn">
                    <div className="text-center mb-6">
                        ...
                        <p className="text-sm text-slate-500">Sube tus fotos para verificar tu identidad manualmente.</p>
                    </div>
                    ...
                    {uploadKycDocsMutation.isError && (
                        <div className="p-4 bg-red-50 rounded-xl text-red-600 text-sm font-medium border border-red-100 flex items-start gap-3">
                            ...
                            <span>{uploadKycDocsMutation.error instanceof Error ? uploadKycDocsMutation.error.message : 'Error al subir los documentos.'}</span>
                        </div>
                    )}
                    ...
                    <div className="flex gap-3">
                        <button 
                            ...
                            disabled={!frontImage || !backImage || !selfieImage || uploadKycDocsMutation.isPending}
            ...
            {step === 2 && (
                <div className="flex flex-col items-center justify-center py-12 space-y-6 animate-fadeIn">
                    ...
                    <div className="text-center">
                        <h3 className="text-xl font-bold text-slate-900 mb-2">Subiendo Documentos</h3>
                        <p className="text-sm text-slate-500">Guardando tus fotos de identificación...</p>
                    </div>
                </div>
            )}
