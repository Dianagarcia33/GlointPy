...
import { bulkUploadInvestors } from '../../../../services/investors';

interface BulkUploadInvestorsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUploaded: () => void;
}

export const BulkUploadInvestorsModal: React.FC<BulkUploadInvestorsModalProps> = ({
  isOpen,
  onClose,
  onUploaded
}) => {
  ...
  const downloadTemplate = () => {
    ...
    link.setAttribute("download", "plantilla_inversionistas.csv");
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h3 className="text-lg font-semibold text-slate-800 flex items-center gap-2">
            ...
            Carga Masiva de Inversionistas
          </h3>
          <button
            onClick={() => {
              setFile(null);
              ...
              {result.success > 0 && (
                <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex gap-3 items-center text-emerald-700">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{result.success} inversionistas creados correctamente.</p>
                  </div>
                </div>
              )}
