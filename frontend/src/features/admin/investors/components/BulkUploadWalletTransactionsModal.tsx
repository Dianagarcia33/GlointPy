...
import { bulkUploadWalletTransactions } from '../../../../services/wallets';

interface BulkUploadWalletTransactionsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUploaded: () => void;
}

export const BulkUploadWalletTransactionsModal: React.FC<BulkUploadWalletTransactionsModalProps> = ({
  isOpen,
  onClose,
  onUploaded
}) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (lines.length <= 1) {
        throw new Error("El archivo está vacío o no contiene transacciones.");
      }
      ...
      for (let i = 0; i < dataLines.length; i += chunkSize) {
        ...
        const response = await bulkUploadWalletTransactions(chunkFile);
  ...
  const downloadTemplate = () => {
    const csvContent = "\uFEFFid;wallet_id;amount;type;reference_type;reference_id;description;balance_after;created_at;updated_at\n1;4;2539972.72;yield_payout;App\\Models\\Retiro;814;Rendimientos de inversión: IG1050;2539972.72;2026-03-01 21:45:46;2026-03-01 21:45:46";
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", "plantilla_transacciones_billetera.csv");
          ...
          {!result?.success && !result?.errors.length && (
            <div className="mb-6 bg-blue-50 border border-blue-100 rounded-lg p-4 flex gap-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <p className="mb-2">Sube un archivo CSV con las columnas del respaldo SQL:</p>
                <ul className="list-disc pl-4 space-y-1 text-blue-700/80 mb-3">
                  <li><strong>id (opcional):</strong> ID numérico de la transacción.</li>
                  <li><strong>wallet_id:</strong> ID numérico de la billetera.</li>
                  <li><strong>amount:</strong> Monto de la transacción (ej: 2539972.72).</li>
                  <li><strong>type:</strong> yield_payout, bonus_payout, pending_payout, etc.</li>
                  <li><strong>reference_type / reference_id (opcional):</strong> Modelo e ID asociado.</li>
                  <li><strong>description:</strong> Glosa de la transacción.</li>
                  <li><strong>balance_after:</strong> Balance resultante (ej: 2539972.72).</li>
                  <li><strong>created_at / updated_at (opcional):</strong> Formato YYYY-MM-DD HH:MM:SS.</li>
                </ul>
                <button 
              ...
              {result.success > 0 && (
                <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-xl flex gap-3 items-center text-emerald-700">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{result.success} transacciones cargadas correctamente.</p>
                  </div>
                </div>
              )}
