...
import { X, User, CreditCard, Landmark, Loader2, Calendar } from 'lucide-react';
...
export const EditInvestmentModal: React.FC<EditInvestmentModalProps> = ({ isOpen, onClose, investment }) => {
    ...
    const [activeTab, setActiveTab] = useState<'personal' | 'bank' | 'financial'>('personal');
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                <div className="flex items-center justify-between p-6 border-b border-slate-100">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">ID: {investment.codigo_asignado || investment.id}</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 bg-slate-50/50">
                    <form id="edit-investment-form" onSubmit={handleSubmit} className="space-y-6">
                        {/* Tabs */}
                        <div className="flex border-b border-slate-200">
                            ...
                            <button type="button" onClick={() => setActiveTab('financial')} className={`flex items-center gap-2 px-6 py-3 font-semibold text-sm border-b-2 transition-colors ${activeTab === 'financial' ? 'border-brand-500 text-brand-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
                                <CreditCard className="w-4 h-4" /> Datos Financieros
                            </button>
                        </div>
