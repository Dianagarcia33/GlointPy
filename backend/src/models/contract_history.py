...
class ContractHistory(Base):
    ...
    paquete_inversion_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("packages.id", ondelete="SET NULL"), nullable=True)
    ...
    dias_contrato: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    updated_at: Mapped[Optional[datetime]] = mapped_column(TIMESTAMP, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=True)
