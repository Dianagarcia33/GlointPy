...
class Acceleration(Base):
    ...
    capital_released: Mapped[Optional[float]] = mapped_column(Numeric(15, 2), default=0.00)
    new_duration: Mapped[float] = mapped_column(Numeric(20, 6), default=0)
    applied: Mapped[bool] = mapped_column(Boolean, default=False)
    bonus_amount: Mapped[Optional[float]] = mapped_column(Numeric(20, 6), default=0)
    created_at: Mapped[Optional[datetime]] = mapped_column(TIMESTAMP, default=datetime.utcnow)
    updated_at: Mapped[Optional[datetime]] = mapped_column(TIMESTAMP, default=datetime.utcnow, onupdate=datetime.utcnow)
