    ...
    try:
        ...
        for row in base_users:
            ...
            if fecha_ingreso and real_fin:
                ...
                if real_fin <= cutoff_date:
                    if real_fin > date(2026, 5, 29):
                        june_capital = capital
                    else:
                        historical_capital = capital
                    capital_liberado = june_capital + historical_capital
            ...
            if historical_capital > 0:
                transactions_to_insert.append(f"INSERT INTO wallet_transactions (wallet_id, amount, type, reference_type, description, balance_after, created_at, updated_at) VALUES ((SELECT id FROM wallets WHERE user_id = {uid}), {historical_capital}, 'CREDIT', 'CAPITAL_LIBERADO_HISTORICO', 'Migración de liberación de capital por finalización de contrato (antes de 29 de mayo)', {calculated_real_balance}, NOW(), NOW());")
    ...
    try:
        ...
        for row in res_inv.fetchall():
            ...
            grouped_data[uid]["inversiones"].append({
                ...
                "estado": row.estado,
                "created_at": row.created_at,
                "fecha_ingreso": row.fecha_ingreso.strftime("%Y-%m-%dT12:00:00") if row.fecha_ingreso else None,
                "fecha_finalizacion": row.fecha_finalizacion.strftime("%Y-%m-%dT12:00:00") if row.fecha_finalizacion else None,
        ...
        for row in res_ret.fetchall():
            ...
            grouped_data[uid]["retiros"].append({
                ...
                "impuesto": row.impuesto,
                "monto_neto": row.monto_neto,
                "fecha_solicitud": row.fecha_solicitud.strftime("%Y-%m-%dT12:00:00") if row.fecha_solicitud else None,
                "fecha_retiro": row.fecha_retiro.strftime("%Y-%m-%dT12:00:00") if row.fecha_retiro else None,
                "estado": row.estado,
                "metodo_pago": row.metodo_pago,
                "banco": row.banco,
                "tipo_cuenta": row.tipo_cuenta,
        ...
        for row in res_hist.fetchall():
            ...
            grouped_data[uid]["histories"].append({
                "id": row.id,
                "fecha_inicio": row.fecha_inicio.strftime("%Y-%m-%dT12:00:00") if row.fecha_inicio else None,
                "fecha_fin": row.fecha_fin.strftime("%Y-%m-%dT12:00:00") if row.fecha_fin else None,
                "dias_contrato": row.dias_contrato,
                "total_contrato": row.total_contrato,
                "tasa_interes": row.tasa_interes,
                "acciones_otorgadas": row.acciones_otorgadas,
